...
d3.csv("sg_weather.csv").then(data => {
    ...
    let data1 = data.filter(d => d.Decade == 1980);

    let chart1 = BeeswarmChart(data1, {
        x: d => d.max_temperature,
        /*group: d => d.Decade, // is this correct? */
        label: "Maximum Temperature →",
        type: d3.scaleLinear, // try d3.scaleLog
        /*title: d => `${d.Year}: ${d.Month}\n${d.maximum_rainfall_in_a_day.toLocaleString("en")} lbs.`,*/
        width: 2000,
        marginTop: 150,
      });

    document.getElementById("chart").appendChild(chart1);

    let data2 = data.filter(d => d.Decade == 1990);

    let chart2 = BeeswarmChart(data2, {
        x: d => d.max_temperature,
        /*group: d => d.Decade, // is this correct? */
        label: "Maximum Temperature →",
        type: d3.scaleLinear, // try d3.scaleLog
    ...
    document.getElementById("chart").appendChild(chart2);

    let data3 = data.filter(d => d.Decade == 2000);

    let chart3 = BeeswarmChart(data3, {
        x: d => d.max_temperature,
        /*group: d => d.Decade, // is this correct? */
        label: "Maximum Temperature →",
        type: d3.scaleLinear, // try d3.scaleLog
    ...
    document.getElementById("chart").appendChild(chart3);

    let data4 = data.filter(d => d.Decade == 2010);

    let chart4 = BeeswarmChart(data4, {
        x: d => d.max_temperature,
        /*group: d => d.Decade, // is this correct? */
        label: "Maximum Temperature →",
        type: d3.scaleLinear, // try d3.scaleLog
    ...
    document.getElementById("chart").appendChild(chart4);

    let data5 = data.filter(d => d.Decade == 2020);

    let chart5 = BeeswarmChart(data, {
        x: d => d.max_temperature,
        group: d => d.Decade, // is this correct? */
        label: "Maximum Temperature →",
        type: d3.scaleLinear, // try d3.scaleLog
    ...
    document.getElementById("chart").appendChild(chart5);
...
function BeeswarmChart(data, {
    ...
    colors = d3.schemePuOr[5], // an array of color strings, for the dots
