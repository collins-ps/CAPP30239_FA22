...
df_old = pd.read_csv('sg_weather.csv')
...
for index, row in df_old.iterrows():
    ...
    df_new["total_rainfall"][int(row["year"]) - 1982] += int(row["total_rainfall"])
    
    max_rain = int(row["maximum_rainfall_in_a_day"])
    ...
    df_new["sum_temp"][int(row["year"]) - 1982] += int(row["mean_temp"])
...
df_new.to_csv('sg_weather_year.csv', index = False)